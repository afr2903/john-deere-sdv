...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  //int angle = (btBuffer[0]-'0')*10000 + (btBuffer[1]-'0')*1000 + (btBuffer[2]-'0')*100 + (btBuffer[3]-'0')*10 + (btBuffer[4]-'0');
	  int angle = (btBuffer[1]-'0')*10 + (btBuffer[2]-'0');
	  int pwm = angle * 1.111;
	  if( btBuffer[0] == '+' )
		  pwm = 100 - pwm;
	  else
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin : CA_Pin */
  GPIO_InitStruct.Pin = CA_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING_FALLING;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(CA_GPIO_Port, &GPIO_InitStruct);

  /*Configure GPIO pin : CB_Pin */
  GPIO_InitStruct.Pin = CB_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(CB_GPIO_Port, &GPIO_InitStruct);
...
/*void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if (HAL_GPIO_ReadPin(CB_GPIO_Port, CB_Pin) == GPIO_PIN_SET){
	  position++;
	} else {
	  position--;
	}
